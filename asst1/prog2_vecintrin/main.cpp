...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  __cs149_mask maskAll, maskIsZero, maskIsNotZero, maskNeedMoreIters, maskNeedClamp;

  for (int i = 0; i < N; i += VECTOR_WIDTH) {
    
    // Handle the case when N is not divisible by VECTOR_WIDTH
    int remaining = N - i;
    ...
    // For exponent == 0, set output to 1.0
    _cs149_vset_float(result, 1.f, maskIsZero);

    // For exponent != 0, compute the power
    maskIsNotZero = _cs149_mask_not(maskIsZero);
    maskIsNotZero = _cs149_mask_and(maskIsNotZero, maskAll);

    // Initialize result to x for non-zero exponents
    _cs149_vmove_float(result, x, maskIsNotZero);
    
    // Initialize count = exponent - 1
    _cs149_vsub_int(count, y, intOne, maskIsNotZero);

    // Loop while any lane still needs iterations
    _cs149_vgt_int(maskNeedMoreIters, count, zero, maskIsNotZero);
    while (_cs149_cntbits(maskNeedMoreIters) > 0) {
      // result *= x for lanes that need more iterations
      _cs149_vmult_float(result, result, x, maskNeedMoreIters);
      
      // count-- for lanes that need more iterations
      _cs149_vsub_int(count, count, intOne, maskNeedMoreIters);
      
      // Update mask for next iteration
      _cs149_vgt_int(maskNeedMoreIters, count, zero, maskIsNotZero);
    }

    // Clamp values > 9.999999f
    _cs149_vgt_float(maskNeedClamp, result, clampValue, maskIsNotZero);
